...
**Three blockers found** (all three produce *silent wrong data* or block flagship datasets):
...
- **B2 — large manifests rejected.** Reading large regions of large arrays fails with "invalid Manifest Flatbuffer". Root cause: the manifest FlatBuffers `Verifier` uses **default `max_tables` (1,000,000)**; GLAD's `lclu` has a 94 MB manifest with >1e6 chunk-ref tables. `icechunk` reads the same region fine. GDAL can therefore read only the portions of GLAD (a flagship public dataset) backed by small-enough manifests. **Round 2 bisected the threshold to exactly 1,000,000 chunk refs per manifest** (a 6.6 MB manifest already fails — it is the table *count*, not size; §6.1).
